...
def PLS_DA(cmode, fold_quantity, components):
    ...
    if cmode in ['binary', 'debug']:
        ...
        if cmode != 'debug':
            ...
            print('===================')
        return acc, sens, spec, f1, np.array(model.x_loadings_), np.array(model.y_loadings_), answers, test_pair_predictions
...
if __name__ == '__main__':
    ...
    fpr, tpr, x = mt.roc_curve(output[6],[-ff[0] for f in output[7] for ff in f],pos_label=0)
    ...
    plt.axhline(color='0.3')
    plt.axvline(color='0.3')
    plt.xlabel('PC 1')
    plt.ylabel('PC 2')
    fig.savefig("Figure2.png", dpi=400)
 
    #plot of number of components versus F1 score
    dd=[]
    for i in range(1,76):
